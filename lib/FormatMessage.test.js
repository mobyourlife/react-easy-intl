...
const I_HAVE_QTY_BOOKS = 'I have {qty, plural, =0{no books} one{1 book} other{# books} }.'
...
it('pluralizes zero', () => {
  const wrapper = render(<FormatMessage str={I_HAVE_QTY_BOOKS} qty={0} />)
  expect(wrapper.text()).toEqual('I have no books.')
})

it('pluralizes one', () => {
  const wrapper = render(<FormatMessage str={I_HAVE_QTY_BOOKS} qty={1} />)
  expect(wrapper.text()).toEqual('I have 1 book.')
})

it('pluralizes number', () => {
  const wrapper = render(<FormatMessage str={I_HAVE_QTY_BOOKS} qty={100} />)
